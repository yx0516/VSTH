...
// 获取源Mac地址
func (p *Packet) GetSrcMac() string {
	return macToString(p.SrcMac)
}

// 获取目标Mac地址
func (p *Packet) GetDestMac() string {
	return macToString(p.DestMac)
}
	...
	if len(headers) == 2 {
		...
		if addr, ok := p.Headers[0].(addrHdr); ok {
			if hdr, ok := p.Headers[1].(addrStringer); ok {
				...
				// TODO:2014-09-28 上级调用函数 String() 已经带了时间，同时带上mac地址
				return fmt.Sprintf("%s\n%s -> %s", hdr.String(addr), p.GetSrcMac(), p.GetDestMac())
			}
		}
	}
...
func (p *Packet) String() string {
	...
	return fmt.Sprintf("%s %s", p.Time.Format("2006-01-02 15:04:05"), p.headerString(p.Headers))
}
