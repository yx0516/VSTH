...
function resultInitial(msg) {
    ...
    if (resultJson.State == -1) {
        ...
        if (resultJson.Data.type == 1) {
            ...
            if (molActivityCnt == 0) {
                _molPropEl.append("<a class='list-group-item'><span class='text-info'>Activities:</span>N/A</a>");
                _molPropEl.append("<a class='list-group-item'><span class='text-info'>References:</span>N/A</a>");
            } else {
                ...
                for (var m = 0; m < molActivityCnt; m++) {
                    ...
                    if (resultJson.Data.references[m].Journal == "") {
                        _molPropEl.append("<a class='list-group-item'><span class='text-info'>Activity-"+n+":</span>" + activityStr + "</a>");
                        _molPropEl.append("<a class='list-group-item'><span class='text-info'>Reference-"+n+":</span>N/A</a>");
                    } else {
                        // 构建文献数据字符串
                        ...
                        _molPropEl.append("<a class='list-group-item'><span class='text-info'>Activity-"+n+":</span>" + activityStr + "</a>");
                        _molPropEl.append("<a class='list-group-item'><span class='text-info'>Reference-"+n+":</span>" + refStr + "</a>");
                    }
                }
            }
        ...
        $(".molIDList a").on({
            click: function() {
                ...
                var molName = $(this).attr("data-id").trim();
                //@bug: 未进入循环
                for (var i = 0, len = resultJson.Data.mols.length; i < len; i++) {
                    if (resultJson.Data.mols[i].MolName.trim().indexOf(molName)>=0) {
                        console.log(molName);
                        defaultMolStr = resultJson.Data.mols[i].MolData + "$$$$\n";
        ...
        $("button[value='Recente']").on({
            click: function() {
                viewer.zoomTo();
            }
        });
        $("button[value='hideQuery']").on({
            click: function() {
                viewer.clear();
                // var v1 = viewer.addModel(queryMolStr, "sdf");
                // 	v1.setStyle({elem:'H', invert: true},{stick:{colorscheme:"greenCarbon",radius:0.15,hidden:false}});
                console.log(defaultMolStr);
                var v2 = viewer.addModel(defaultMolStr, "sdf");
                v2.setStyle({ elem: 'H', invert: true }, { stick: { radius: 0.15 } });
                viewer.zoomTo();
                viewer.render();
        ...
        $("button[value='showQuery']").on({
            click: function() {
                viewer.clear();
                console.log(queryMolStr);
                console.log(defaultMolStr);
                var v1 = viewer.addModel(queryMolStr, "sdf");
                v1.setStyle({ elem: 'H', invert: true }, { stick: { colorscheme: "greenCarbon", radius: 0.15, hidden: false } });
...
$(".js-downloadMol").on({
    click: function() {
        ...
        var url = 'downloadJobResult?jobId=' + jobID;
