...
function resultInitial(msg) {
    ...
    if (resultJson.State == -1) {
        ...
        var glviewer = $3Dmol.createViewer(element1, config);
        var viewer2 = $3Dmol.createViewer(element2, config);

        // glviewer ：显示蛋白质
        var v1 = glviewer.addModel(resultJson.Data.PDB, "pdb");
        v1.setStyle({}, { cartoon: { opacity: 0.6, arrows: true } })

        var v2 = glviewer.addModel(defaultMolStr, "sdf");
        v2.setStyle({}, { stick: { colorscheme: "greenCarbon", radius: 0.15 } });

        selectModel = defaultMolStr;
        ...
        // molviewer ：显示小分子
        var v = viewer2;
        v.addModel(defaultMolStr, "sdf");
        v.setStyle({}, { stick: { colorscheme: "greenCarbon", radius: 0.15 } });
        v.zoomTo();
        v.render();
        ...
        $(".molIDList a").on({
            click: function() {
                ...
                var v2 = glviewer.addModel(molStr, "sdf");
                v2.setStyle({}, { stick: { colorscheme: "greenCarbon", radius: 0.15 } });
        ...
        $("button[value='Sphere']").on({
            click: function() {
                glviewer.setStyle({}, { sphere: {} });
                glviewer.render();
            }
        });
        $("button[value='Cross']").on({
            click: function() {
                glviewer.setStyle({}, { cross: { linewidth: 2 } });
                glviewer.render();
            }
        });
        $("button[value='Cartoon']").on({
            click: function() {
                glviewer.setStyle({ hetflag: false }, { cartoon: { color: 'spectrum' } });
                glviewer.render();
            }
        });
        $("button[value='Surface']").on({
            click: function() {
                glviewer.addSurface();
                glviewer.render();
            }
        });
        $("button[value='RM-Surface']").on({
            click: function() {
        ...
        $("button[value='Default']").on({
            click: function() {
                glviewer.clear();

                var v1 = glviewer.addModel(resultJson.Data.PDB, "pdb");
                v1.setStyle({}, { cartoon: { opacity: 0.7 } });

                var v2 = glviewer.addModel(selectModel, "sdf");
                v2.setStyle({}, { stick: { colorscheme: "greenCarbon", radius: 0.15 } });
